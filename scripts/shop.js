// PokeChill: Shop and item purchase logic.
const shop = {}

/**
 * Registra na Poke-Mart as 20 trocas mítico (100 pokebolas → 1 Pokémon).
 * Implementação em scripts/mythicTrades.js → registerMythicShopEntries()
 */
registerMythicShopEntries()
...
shop.shopGymBadgeBrockTicket = {
    ...
    price: 1,
    currency: item.gymBadgeBrock.id,
    category: `goods`,
    condition: function() { return item.gymBadgeBrock.got >= 1 },
    effect: function() {
        item.autoRefightTicket.got++
        item.autoRefightTicket.newItem++
    },
}
shop.shopGymBadgeMistyTicket = {
    icon: item.autoRefightTicket.id,
    name: `Auto-Refight Ticket`,
    price: 1,
    currency: item.gymBadgeMisty.id,
    category: `goods`,
    condition: function() { return item.gymBadgeMisty.got >= 1 },
    effect: function() {
        item.autoRefightTicket.got++
        item.autoRefightTicket.newItem++
    },
...
function updateItemShop(){
    ...
    `;} else {div.innerHTML = `
    <img src="${getItemImageSrc(shop[i].icon)}">
        <span>${name}${stockTag}</span>
    <strong id="shop-currency-${i}">
        <img src="${getItemImageSrc(currency)}">
        x${shop[i].price}
    </strong>
    `}
    ...
    const leftImg = shop[i].sprite
        ? `img/pkmn/sprite/${shop[i].sprite}.png`
