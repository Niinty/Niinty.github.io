// PokeChill: Exploration menu and area selection logic.
...
function format(input) {
    ...
    if (item[input]?.name) str = String(item[input].name);
...
let dailyCatchPokemon
let dailyCatchAttempts = 0
let dailyCatchStatusMessage = ""
const DAILY_CATCH_MAX_ATTEMPTS = 3
...
for (let i = 0; i < 4; i++) {
    ...
    hpMultiplier = 1000000;
        ...
        if (currentTrainerSlot>maxTrainerSlot) { //trainer won
            ...
            const trainerArea = areas[saved.currentArea]
            const isGymLeaderWin = trainerArea.isGym === true
        ...
        if (areas[saved.currentArea].isGym) {
            wildLevel = getGymEffectiveLevel(areas[saved.currentArea])
            hpMultiplier = getGymEffectiveHpMultiplier(areas[saved.currentArea])
        } else {
...
function updateItemsGot(){
    ...
    for (const i in item) {
        ...
        if (item[i]?.hidden && i !== "mysteryEgg") continue
        if (!item[i]?.newItem) continue
...
// tryDropMythicBall() — definida em scripts/mythicTrades.js
...
function leaveCombat(){
    ...
    //Remove o fight again do Gym
    if ( areas[saved.currentArea].isGym ) document.getElementById("area-rejoin").style.display = "none"
    ...
    if (pkmn[hatchedPkmn].shiny==true && giveStarsign(hatchedPkmn,`check`) != "complete" && rng(1/8042)){ //starsign
    ...
    // Limpa o carimbo para que o Pokémon fique normal na sua Box
    pkmn[i].isGymReward = undefined;
    ...
    if ( areas[saved.currentArea]?.isLegendsBattle ) {
            ...
            Auto-Refight <span>(Costs <img src="img/items/bottleCap.png"> 01 Bottle Caps — possui ${item.bottleCap.got})</span>
            `
        }
...
for (let i = activeBars; i < hpBars.length; i++) {
  hpBars[i].el.style.display = "none";
    ...
    // Loot ao derrotar inimigo: itens do mapa (regras antigas) + pokebolas míticas (exceto Treino)
    if (saved.currentArea && areas[saved.currentArea]) {
        if (rng(0.20) && !areas[saved.currentArea]?.trainer && saved.currentArea != areas.frontierSpiralingTower.id && saved.currentArea != areas.training.id) {
            dropItem()
        }
        if (tryDropMythicBall()) updateItemsGot()
    }
...
function openMenu(){
    ...
    if (saved.tutorialStep != "none")  {
        ...
        document.getElementById(`menu-item-gyms`).className = `menu-item menu-item-locked`
        ...
        document.getElementById(`menu-item-gyms`).className = `menu-item`
...
function exploreCombatPlayer() {
    ...
    if (barProgressPlayer < 99) {
        ...
        // Apply default fatigue multiplier (reduce fatigue by 25%)
        if (typeof testFatigueMultiplier === 'number') fatigueDamage *= testFatigueMultiplier
    ...
    if (shinyMark==true) completedFlair = `<svg style="color: rgb(209, 130, 11); opacity: 0.7" class="completed-flair" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15"><path fill="currentColor" d="M14 1v1.5c-.75 0-.75 1.5 0 1.5v1.25c-.75 0-.75 1.5 0 1.5v1.5c-.75 0-.75 1.5 0 1.5V11c-.75 0-.75 1.5 0 1.5V14h-1.5c0-.75-1.5-.75-1.5 0H9.75c0-.75-1.5-.75-1.5 0h-1.5c0-.75-1.5-.75-1.5 0H4c0-.75-1.5-.75-1.5 0H1v-1.5c.75 0 .75-1.5 0-1.5V9.75c.75 0 .75-1.5 0-1.5v-1.5c.75 0 .75-1.5 0-1.5V4c.75 0 .75-1.5 0-1.5V1h1.5c0 .75 1.5.75 1.5 0h1.25c0 .75 1.5.75 1.5 0h1.5c0 .75 1.5.75 1.5 0H11c0 .75 1.5.75 1.5 0zm-2 2H3v9h9zm-1 1v7H4V4z"/></svg>`
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span style="font-size:1.2rem">${areas[i].name || format(i)}</span>
...
function setEventAreas() {
  ...
  for (const i in areas) {
       ...
       if (areas[i].encounter && areas[i].difficulty===tier4difficulty && areas.vsTeamLeaderGiovanni.defeated!=true) unlockRequirement =`<span class="ticket-unlock">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2 16c0-2.828 0-4.243.879-5.121C3.757 10 5.172 10 8 10h8c2.828 0 4.243 0 5.121.879C22 11.757 22 13.172 22 16s0 4.243-.879 5.121C20.243 22 18.828 22 16 22H8c-2.828 0-4.243 0-5.121-.879C2 20.243 2 18.828 2 16" opacity="0.5"/><path fill="currentColor" d="M6.75 8a5.25 5.25 0 0 1 10.5 0v2.004c.567.005 1.064.018 1.5.05V8a6.75 6.75 0 0 0-13.5 0v2.055a24 24 0 0 1 1.5-.051z"/></svg>
       Defeat Team Leader Giovanni in VS to unlock</span>`
       ...
       if (areas[i].encounter && areas[i].difficulty===tier5difficulty) eventTag = `<strong style="filter:hue-rotate(260deg)" class="event-tag">Tier V Raid ❖</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier6difficulty) eventTag = `<strong style="filter:hue-rotate(320deg)" class="event-tag">Tier VI Raid ❖</strong>`
...
function switchMenu(id){
    ...
    if (/vs|gyms|items|team|dex|dictionary|guide/.test(id) && saved.tutorialStep != "none") {
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
    ...
    if (id=="shop" && areas.vsGymLeaderBrock.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Defeat Gym Leader Brock in Ginásios to unlock`
        openTooltip()
        return
    }
    ...
    if (id=="training" && areas.vsGymLeaderMisty.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = `Defeat Gym Leader Misty in Ginásios to unlock`
        openTooltip()
        return
    }
    ...
    document.getElementById(`gyms-menu`).style.zIndex = "30"
    ...
    if (id==="gyms") {
        if (saved.currentArea!==undefined) {openMenu(); return; }
        document.getElementById(`gyms-menu`).style.display = "flex"
        document.getElementById(`gyms-menu`).style.zIndex = "40"
        updateGyms()
    }
...
function isDailyCatchUnlocked(){
    return saved.tutorialStep === `none`
}

function isDailyCatchAvailable(){
    if (!isDailyCatchUnlocked()) return false
    const halfDayNumber = currentDailyCatchHalfDay()
    return saved.dailyCatchLastUsedHalfDay !== halfDayNumber
}

function hasAnyPokemonAtLevel(level){
    for (const i in pkmn){
        if (!pkmn[i]) continue
        if (pkmn[i].caught > 0 && typeof pkmn[i].level === `number` && pkmn[i].level >= level) return true
    }
    return false
}

function resetStoredDailyCatchIfExpired(){
    const halfDayNumber = currentDailyCatchHalfDay()
    if (saved.dailyCatchPokemonHalfDay !== halfDayNumber){
        saved.dailyCatchPokemon = undefined
        saved.dailyCatchPokemonHalfDay = undefined
        saved.dailyCatchAttempts = 0
        dailyCatchAttempts = 0
    }
}
...
function getStoredDailyCatchPokemon(){
    resetStoredDailyCatchIfExpired()
    if (!isDailyCatchAvailable()){
        if (dailyCatchStatusMessage && saved.dailyCatchPokemon) return saved.dailyCatchPokemon
        return undefined
    }
    if (isMythicTradePokemon(saved.dailyCatchPokemon)){
        ...
        saved.dailyCatchPokemonHalfDay = undefined
        saved.dailyCatchAttempts = 0
        dailyCatchPokemon = undefined
        dailyCatchAttempts = 0
        return undefined
    }
    if (saved.dailyCatchPokemonHalfDay === currentDailyCatchHalfDay()) return saved.dailyCatchPokemon
    return undefined
}
...
function getDailyCatchRank(poke){
    const pkmnData = pkmn[poke]
    const division = pkmnData ? returnPkmnDivision(pkmnData) : undefined
    if (division) {
        if (division === `D` || division === `C`) return `common`
        if (division === `B`) return `uncommon`
        if (division === `A`) return `rare`
        if ([`S`, `SS`, `SSS`, `SSS+`].includes(division)) return `legendary`
    }
    const bst = pkmnData?.bst
    if (!bst) return `common`
    const total = Object.values(bst).reduce((sum, value) => sum + (value || 0), 0)
    if (total >= 540) return `legendary`
    if (total >= 460) return `rare`
...
function getDailyCatchRankWeight(rank){
    const weights = {
        common: 10,
        uncommon: 5,
        rare: 2,
...
function improveOwnedDailyCatchIv(poke){
    const ivs = pkmn[poke]?.ivs
    if (!ivs) return null
    const available = Object.keys(ivs).filter(stat => ivs[stat] < 6)
    if (available.length === 0) return null
    const stat = arrayPick(available)
    ivs[stat] = Math.min(6, ivs[stat] + 1)
    pkmn[poke].dictionaryTagIvSum = ivs.hp + ivs.atk + ivs.satk + ivs.spe + ivs.sdef + ivs.def
    return stat
}

function chooseDailyCatchPokemon(){
    const weightedPool = []
    const blockLegendary = !hasAnyPokemonAtLevel(50)
    for (const i in pkmn){
        if (pkmn[i].tagObtainedIn == `unobtainable`) continue
        if (pkmn[i].hidden == true) continue
        if (pkmn[i].tagObtainedIn === undefined) continue
        if (isMythicTradePokemon(i)) continue
        const rank = getDailyCatchRank(i)
        if (blockLegendary && rank === `legendary`) continue
        const weight = getDailyCatchRankWeight(rank)
        if (weight <= 0) continue
        weightedPool.push({ id: i, weight })
    }
    if (weightedPool.length == 0) return `magikarp`
    return weightedPickByWeight(weightedPool).id
}

function claimDailyCatch(){
    if (!isDailyCatchUnlocked()){
        document.getElementById(`tooltipMid`).innerHTML = `Complete o tutorial primeiro para desbloquear o Daily Catch.`
        openTooltip()
        return
    }
    if (!isDailyCatchAvailable()){
        document.getElementById(`tooltipMid`).innerHTML = `Daily Catch está em cooldown. Aguarde ${getDailyCatchRemainingText()}`
        openTooltip()
        return
    }
    resetStoredDailyCatchIfExpired()
    dailyCatchStatusMessage = ""
    if (!saved.dailyCatchPokemon || saved.dailyCatchPokemonHalfDay !== currentDailyCatchHalfDay()){
        saved.dailyCatchPokemon = chooseDailyCatchPokemon()
        saved.dailyCatchPokemonHalfDay = currentDailyCatchHalfDay()
        saved.dailyCatchAttempts = 0
        dailyCatchAttempts = 0
    } else {
    ...
    openMenu()
    document.getElementById(`daily-catch-menu`).style.display = `flex`
}

function updateDailyCatchMenu(){
    const menu = document.getElementById(`daily-catch-menu`)
    if (!menu) return
    resetStoredDailyCatchIfExpired()
    const available = isDailyCatchAvailable()
    const storedPoke = getStoredDailyCatchPokemon()
    if (available && !storedPoke){
        saved.dailyCatchPokemon = chooseDailyCatchPokemon()
        saved.dailyCatchPokemonHalfDay = currentDailyCatchHalfDay()
        saved.dailyCatchAttempts = 0
        dailyCatchAttempts = 0
    }
    dailyCatchPokemon = getStoredDailyCatchPokemon()
    dailyCatchAttempts = saved.dailyCatchAttempts ?? 0
    const triesLeft = DAILY_CATCH_MAX_ATTEMPTS - dailyCatchAttempts
    ...
    document.querySelectorAll(`.daily-catch-ball-button`).forEach(button => {
        ...
        const count = item[ballId]?.got || 0
        button.innerHTML = `${button.dataset.ballLabel}\n(${count})`
        const disabled = !(count > 0 && available && triesLeft > 0)
        button.style.opacity = disabled ? `0.45` : `1`
        button.classList.toggle(`disabled`, disabled)
    })
    document.querySelectorAll(`.time-counter-daily`).forEach(el => {
        el.textContent = getDailyCatchRemainingText()
    })
}

function attemptDailyCatch(ballId){
    if (!isDailyCatchUnlocked()){
        document.getElementById(`daily-catch-message`).textContent = `Complete o tutorial primeiro para desbloquear o Daily Catch.`
        return
    }
    if (!isDailyCatchAvailable()){
        claimDailyCatch()
    ...
    if (dailyCatchAttempts >= DAILY_CATCH_MAX_ATTEMPTS){
        document.getElementById(`daily-catch-message`).textContent = `Sem tentativas restantes. Aguarde ${getDailyCatchRemainingText()}`
        return
    }
    const ballItem = item[ballId]
    if (!ballItem || ballItem.got <= 0){
        document.getElementById(`daily-catch-message`).textContent = `Você precisa de pelo menos uma ${format(ballId)}.`
        return
    }
    ballItem.got--
    dailyCatchAttempts++
    saved.dailyCatchAttempts = dailyCatchAttempts

    const currentHalfDay = currentDailyCatchHalfDay()
    ...
    if (pkmn[dailyCatchPokemon].caught > 0){
        ...
        if (dailyCatchAttempts >= DAILY_CATCH_MAX_ATTEMPTS){
            saved.dailyCatchLastUsedHalfDay = currentHalfDay
            saved.dailyCatchPokemon = undefined
            saved.dailyCatchPokemonHalfDay = undefined
            saved.dailyCatchAttempts = 0
            dailyCatchPokemon = undefined
    ...
    const rank = getDailyCatchRank(dailyCatchPokemon)
    const chance = getDailyCatchChance(rank, ballId)
    const success = Math.random() < chance
    if (success || ballId === `masterball`){
        const caughtPokemon = dailyCatchPokemon
        givePkmn(pkmn[caughtPokemon], 1)
        saved.dailyCatchLastUsedHalfDay = currentHalfDay
        saved.dailyCatchAttempts = 0
        dailyCatchStatusMessage = `Parabéns pela captura! Você pegou ${format(caughtPokemon)} com ${format(ballId)}!`
        updateItemBag()
        updateDailyCatchMenu()
        return
    }
    const currentPoke = dailyCatchPokemon
    if (dailyCatchAttempts >= DAILY_CATCH_MAX_ATTEMPTS){
        saved.dailyCatchLastUsedHalfDay = currentHalfDay
        saved.dailyCatchAttempts = 0
        dailyCatchStatusMessage = `Não foi dessa vez! Você usou todas as tentativas. Volte em ${getDailyCatchRemainingText()}.`
    } else {
        dailyCatchStatusMessage = `Não foi dessa vez! ${DAILY_CATCH_MAX_ATTEMPTS - dailyCatchAttempts} tentativa${dailyCatchAttempts == 1 ? `` : `s`} restante${dailyCatchAttempts == 1 ? `` : `s`}.`
    }
    updateItemBag()
    updateDailyCatchMenu()
}
...
window.addEventListener('load', function() {
    ...
    saved.wonderTradeClaimed ??= false;
    saved.dailyCatchLastUsedHalfDay ??= undefined;
    saved.dailyCatchPokemon ??= undefined;
    saved.dailyCatchPokemonHalfDay ??= undefined;
    saved.dailyCatchAttempts ??= 0;
