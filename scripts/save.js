// PokeChill: Save/load game persistence and save file behavior.
...
function saveGame() {
  ...
  for (const i in areas) {
    ...
    if (areas[i].isGym) {
      data[i].gymDefeatCount = areas[i].gymDefeatCount;
      data[i].gymBaseLevel = areas[i].gymBaseLevel;
    }
...
function loadGame() {
  ...
  for (const i in item) {
    if (data[i]) {
      item[i].got = data[i].got ?? 0;
      item[i].newItem = data[i].newItem ?? 0;
    }
  }
  ...
  for (const i in areas) {
    ...
    if (areas[i].isGym) {
      if (data[i].gymDefeatCount !== undefined) areas[i].gymDefeatCount = data[i].gymDefeatCount;
      if (data[i].gymBaseLevel !== undefined) areas[i].gymBaseLevel = data[i].gymBaseLevel;
    }
