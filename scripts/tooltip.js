...
frontierArray.sort((a, b) => a.data.tier - b.data.tier);
    ...
    if (category=="item") {
        ...
        else  document.getElementById("tooltipTop").innerHTML = `<img src="${typeof getItemImageSrc === "function" ? getItemImageSrc(ttdata) : `img/items/${ttdata}.png`}">`
        ...
        if (item[ttdata].type==="held" || item[ttdata].heldBonusPower){
            ...
            let rarityLine = ""
            if (item[ttdata].rarity === "epic") rarityLine = `Epic · `
            else if (item[ttdata].rarity === "rare") rarityLine = `Rare · `
            else if (item[ttdata].rarity === "uncommon") rarityLine = `Uncommon · `
            else if (item[ttdata].rarity === "common") rarityLine = `Common · `
            document.getElementById("tooltipMid").innerHTML = `${rarityLine}${item[ttdata].got} in bag`
        }
  ...
  .sort((a, b) => {
   ...
   if (category == "dictionaryMove") {
    ...
    if (category == "dictionaryItem") {
        ...
        const tooltipInfo = item[ttdata].info()
        const tooltipCount = tooltipInfo.includes("(You have") || tooltipInfo.includes("(Possui")
            ? ""
            : `<br>(You have ${item[ttdata].got})`
        document.getElementById("tooltipMid").innerHTML = `${tooltipInfo}${tooltipCount}`
        ...
        // Do not show unobtainable fallback text here; item sources may not be fully enumerated.
